...
public class HangingBridgeBehaviour extends StateBehavior {
	...
	public void action() {
		...
		difference = 200;
		/*
		 * while (Math.abs(difference) > 0.2f && this.hal.getCurrentDistance() <
		 * 20.f && !suppressed) { this.hal.forward(); Delay.msDelay(400);
		 * difference = this.hal.getMeanDistance() - distance; if
		 * (Math.abs(difference) > 0.3f) { this.hal.turn((int)
		 * Math.signum(difference)); Delay.msDelay(200); } }
		 */
